// Modules to control application life and create native browser window
var _a = require('electron'), app = _a.app, BrowserWindow = _a.BrowserWindow, ipcMain = _a.ipcMain, Menu = _a.Menu, MenuItem = _a.MenuItem, dialog = _a.dialog;
var os = require('os');
import { existsSync, mkdirSync, readdirSync, readFileSync, writeFileSync } from 'fs';
import { ProjectBusiness } from './project-business';
import { StageBusiness } from './stage-business';
import { PageBusiness } from './page-business';
import { ControlBusiness } from './control-business';
import { Const } from './const/const';
var MessageMain = /** @class */ (function () {
    function MessageMain() {
        this.projectBusiness = new ProjectBusiness();
        this.stageBusiness = new StageBusiness();
        this.pageBusiness = new PageBusiness();
        this.controlBusiness = new ControlBusiness();
        this.const = new Const();
        if (!existsSync(this.const.resource_base)) {
            mkdirSync(this.const.resource_base);
        }
        if (!existsSync(this.const.home_path)) {
            mkdirSync(this.const.home_path);
        }
        if (!existsSync(this.const.runtime_tmp_path)) {
            mkdirSync(this.const.runtime_tmp_path);
        }
        if (!existsSync(this.const.home_path + '/ud_project.json')) {
            writeFileSync(this.const.home_path + '/ud_project.json', JSON.stringify([]));
        }
    }
    MessageMain.prototype.init = function () {
        var that = this;
        ipcMain.on('ipc-sync-table', function (event, paramObj) {
            that.stageBusiness.syncTable(paramObj);
            event.sender.send('ipc-sync-table-back', {
                status: 'success',
            });
        });
        // =================================
        ipcMain.on('ipc-scan-sub-comp', function (event, projectObj) {
            var treeData = that.controlBusiness.scanSubComp(projectObj);
            event.sender.send('ipc-scan-sub-comp-back', {
                status: 'success',
                data: treeData
            });
        });
        ipcMain.on('ipc-scan-routing', function (event, projectObj) {
            var curNodeFolder = that.controlBusiness.scanRouting(projectObj);
            event.sender.send('ipc-scan-routing-back', {
                status: 'success',
                data: curNodeFolder
            });
        });
        // ---------------------------------
        ipcMain.on('ipc-get-comp-dom-list', function (event, compType) {
            var compDom = that.controlBusiness.getCompDomList(compType);
            event.sender.send('ipc-get-comp-dom-list-back', {
                status: 'success',
                data: compDom
            });
        });
        // =================================
        ipcMain.on('ipc-remove-block', function (event, blockObj) {
            that.pageBusiness.removeBlock(blockObj);
            event.sender.send('ipc-remove-block-back', {
                status: 'success',
            });
        });
        ipcMain.on('ipc-create-block', function (event, blockObj) {
            that.pageBusiness.createBlock(blockObj);
            event.sender.send('ipc-create-block-back', {
                status: 'success',
            });
        });
        // ---------------------------------
        ipcMain.on('ipc-remove-page', function (event, pageObj) {
            that.pageBusiness.removePage(pageObj);
            event.sender.send('ipc-remove-page-back', {
                status: 'success',
            });
        });
        ipcMain.on('ipc-create-page', function (event, pageObj) {
            that.pageBusiness.createPage(pageObj);
            event.sender.send('ipc-create-page-back', {
                status: 'success',
            });
        });
        // ---------------------------------
        ipcMain.on('ipc-remove-folder', function (event, folderObj) {
            that.pageBusiness.removeFolder(folderObj);
            event.sender.send('ipc-remove-folder-back', {
                status: 'success',
            });
        });
        ipcMain.on('ipc-create-folder', function (event, folderObj) {
            that.pageBusiness.createFolder(folderObj);
            event.sender.send('ipc-create-folder-back', {
                status: 'success',
            });
        });
        // =================================
        ipcMain.on('ipc-query-block-all', function (event, tableName) {
            var rows = that.stageBusiness.queryBlockAll();
            event.sender.send('ipc-query-block-all-back', {
                status: 'success',
                data: rows
            });
        });
        ipcMain.on('ipc-update-page-schema', function (event, pageVO) {
            that.stageBusiness.updatePageSchema(pageVO);
            event.sender.send('ipc-update-page-schema-back', {
                status: 'success',
            });
        });
        ipcMain.on('ipc-query-comp-all', function (event, project) {
            var compList = that.stageBusiness.queryCompAll();
            event.sender.send('ipc-query-comp-all-back', {
                status: 'success',
                data: compList
            });
        });
        ipcMain.on('ipc-query-model-all', function (event, project) {
            var modelList = that.stageBusiness.queryModelAll();
            event.sender.send('ipc-query-model-all-back', {
                status: 'success',
                data: modelList
            });
        });
        ipcMain.on('ipc-query-demo-data-all', function (event, project) {
            var demoDataList = that.stageBusiness.queryDemoDataAll();
            event.sender.send('ipc-query-demo-data-all-back', {
                status: 'success',
                data: demoDataList
            });
        });
        //        queryDemoDataAll
        // =================================
        ipcMain.on('ipc-update-project-data', function (event, project) {
            that.projectBusiness.updateProjectData(project);
            event.sender.send('ipc-update-project-data-back', {
                status: 'success',
            });
        });
        ipcMain.on('ipc-get-page-schema', function (event, project) {
            var pageSchema = that.projectBusiness.getPageSchema(project);
            event.sender.send('ipc-get-page-schema-back', {
                status: 'success',
                data: pageSchema
            });
        });
        ipcMain.on('ipc-get-project-tree', function (event, project) {
            var projectSchema = that.projectBusiness.getProjectTree(project);
            event.sender.send('ipc-get-project-tree-back', {
                status: 'success',
                data: projectSchema
            });
        });
        ipcMain.on('ipc-check-project', function (event, project) {
            var retObj = that.projectBusiness.checkProject(project);
            // 需要在checkProject后执行，因为要初始化目录和文件
            that.controlBusiness.scanProject(project);
            event.sender.send('ipc-check-project-back', {
                status: 'success',
                data: retObj
            });
        });
        // ---------------------------------
        ipcMain.on('ipc-remove-project', function (event, project) {
            that.projectBusiness.removeProject(project);
            event.sender.send('ipc-remove-project-back', {
                status: 'success',
            });
        });
        ipcMain.on('ipc-create-project', function (event, project) {
            var projectObj = that.projectBusiness.createProject(project);
            event.sender.send('ipc-create-project-back', {
                status: 'success',
                data: projectObj
            });
        });
        ipcMain.on('ipc-get-project-list', function (event, project) {
            var projectList = that.projectBusiness.getProjectList();
            event.sender.send('ipc-get-project-list-back', {
                status: 'success',
                data: projectList
            });
        });
        // ---------------------------------
        ipcMain.on('ipc-choose-disk-folder', function (event) {
            dialog.showOpenDialog({
                properties: ['openFile', 'openDirectory']
            }, function (files) {
                if (files) {
                    event.sender.send('ipc-choose-disk-folder-back', files);
                }
            });
        });
        ipcMain.on('ipc-open-project', function (event) {
            dialog.showOpenDialog({
                properties: ['openFile', 'openDirectory']
            }, function (files) {
                var retObj;
                if (files && files.length > 0) {
                    var file_path = files[0] + '/package.json';
                    var packageSchema = {};
                    if (existsSync(file_path)) {
                        var json_data = readFileSync(file_path, { encoding: 'utf-8' });
                        packageSchema = JSON.parse(json_data);
                    }
                    if (packageSchema.name) {
                        retObj = {
                            project_name: packageSchema.name,
                            root_path: files[0],
                            status: 'success'
                        };
                    }
                }
                if (!retObj) {
                    retObj = { status: 'error' };
                }
                event.sender.send('ipc-open-project-back', retObj);
            });
        });
        ipcMain.on('ipc-get-resource-path', function (event) {
            var files = readdirSync(os.homedir());
            var list = [];
            files.forEach(function (file, index) {
                list.push(file);
            });
            event.sender.send('ipc-get-resource-path-back', { status: 'success', data: { list: list } });
        });
    };
    return MessageMain;
}());
export { MessageMain };
//# sourceMappingURL=message-main.js.map