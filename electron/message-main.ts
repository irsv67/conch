...
export class MessageMain {
    ...
    init() {
        ...
        ipcMain.on('ipc-scan-sub-comp', (event, projectObj) => {
            const treeData = that.controlBusiness.scanSubComp(projectObj);
        ...
        ipcMain.on('ipc-scan-routing', (event, projectObj) => {
            const curNodeFolder = that.controlBusiness.scanRouting(projectObj);
        ...
        ipcMain.on('ipc-get-comp-dom-list', (event, compType) => {
            const compDom = that.controlBusiness.getCompDomList(compType);
        ...
        ipcMain.on('ipc-query-block-all', (event, tableName) => {
            const rows = that.stageBusiness.queryBlockAll();
        ...
        ipcMain.on('ipc-query-comp-all', (event, project) => {
            const compList = that.stageBusiness.queryCompAll();
        ...
        ipcMain.on('ipc-query-model-all', (event, project) => {
            const modelList = that.stageBusiness.queryModelAll();
        ...
        ipcMain.on('ipc-query-demo-data-all', (event, project) => {
            const demoDataList = that.stageBusiness.queryDemoDataAll();
...
//        queryDemoDataAll
        ...
        ipcMain.on('ipc-get-page-schema', (event, project) => {
            const pageSchema = that.projectBusiness.getPageSchema(project);
        ...
        ipcMain.on('ipc-get-project-tree', (event, project) => {
            const projectSchema = that.projectBusiness.getProjectTree(project);
        ...
        ipcMain.on('ipc-check-project', (event, project) => {
            ...
            const retObj: any = that.projectBusiness.checkProject(project);
        ...
        ipcMain.on('ipc-create-project', (event, project) => {
            const projectObj = that.projectBusiness.createProject(project);
        ...
        ipcMain.on('ipc-get-project-list', (event, project) => {
            const projectList = that.projectBusiness.getProjectList();
        ...
        ipcMain.on('ipc-open-project', (event) => {
            dialog.showOpenDialog({
                ...
                if (files && files.length > 0) {
                    const file_path = files[0] + '/package.json';

                    let packageSchema: any = {};
                    if (existsSync(file_path)) {
                        const json_data = readFileSync(file_path, {encoding: 'utf-8'});
        ...
        ipcMain.on('ipc-get-resource-path', (event) => {
            const files = readdirSync(os.homedir());
            const list = [];
